...
def lasso_obj(beta, lam):
    resid = y - X @ beta
    return (resid @ resid) / M + lam * (np.abs(beta).sum())

def subgrad(beta, lam):
...
for lam in lams:
    ...
    Z_path = np.array([lasso_obj(b, lam) for b in steps])
    ...
    for i in range(B1.shape[0]):
        for j in range(B1.shape[1]):
            Z[i, j] = lasso_obj(np.array([B1[i, j], B2[i, j]]), lam)

    # ----- Static 3D surface + path -----
    eps = 0.02 * (Z.max() - Z.min())  # small z-offset to lift the path above surface
    fig = plt.figure(figsize=(8, 6))
    ax = fig.add_subplot(111, projection="3d")
    ax.plot_surface(B1, B2, Z, linewidth=0, antialiased=True, alpha=0.35)
    ...
    ax.set_title(f"LASSO Surface + Subgradient Path (λ={lam})")
    ax.set_xlabel(r"$\beta_1$")
    ax.set_ylabel(r"$\beta_2$")
    ax.set_zlabel("Objective")
    ax.view_init(elev=32, azim=-40)
    plt.tight_layout()
    plt.savefig(f"lasso_3d_surface_path_lambda{lam}.png", dpi=180)
    plt.close(fig)
